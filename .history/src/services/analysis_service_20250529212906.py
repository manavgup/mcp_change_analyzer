...
class AnalysisService:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_files_per_analysis = self.analysis_config.get(
    ...
    async def analyze_repository(
        ...
        try:
            ...
            changed_files_result = await self.git_service.get_changed_files(
                repo_path=repo_path, include_untracked=include_untracked
            )
            ...
            directory_structure = await self.analyze_directory_structure(
                repo_path=repo_path, max_depth=3
            )

            # Analyze patterns in changed files
            patterns = await self.analyze_patterns(
                repo_path=repo_path, changed_files=changed_files_result["files"]
            )

            # Combine all results
            result = {
                "repository": {"path": repo_path, "metrics": metrics_result},
            ...
            if verbose > 0:
                result["verbose"] = {"level": verbose, "config": self.analysis_config}
    ...
    async def analyze_patterns(
        self, repo_path: str, changed_files: Dict[str, List[Any]]
    ) -> Dict[str, Any]:
        """
    ...
    async def _build_directory_tree(
        self, repo_path: str, max_depth: int
    ) -> Dict[str, Any]:
    ...
    async def _analyze_directory_relationships(
        ...
        relationships = {"parent_child": [], "siblings": []}
    ...
    async def _identify_logical_groups(
        ...
        for prefix, group_files in prefix_groups.items():
            if len(group_files) > 1:  # Only include groups with multiple files
                logical_groups.append(
                    {"name": prefix, "files": group_files, "count": len(group_files)}
                )
